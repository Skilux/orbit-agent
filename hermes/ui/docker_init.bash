...
# Orbit: seed the named-volume Hermes home from the read-only repo config.
# The home lives on a native-fs Docker volume (not the macOS bind-mount) so the
# gateway's advisory lock (flock) is enforced cross-process and the WebUI
# correctly detects the gateway as running. Config (profile, SOUL, skills,
# MCP registrations, token) is refreshed from the repo on each boot; runtime
# files already in the volume (sessions, logs, gateway pid/lock) are preserved.
if [ -d /seed/hermes-home ]; then
...
# scheduled Tasks tick. Runs as the same user (hermeswebui) after deps are
# installed, sharing this container's HERMES_HOME and .env. Messaging
# platforms are disabled (no tokens in .env) — this is cron-only.
echo ""; echo "== Starting Hermes gateway (cron scheduler, background)"
HERMES_HOME="${HERMES_HOME:-/home/hermeswebui/.hermes}" \
  /app/venv/bin/python -m hermes_cli.main -p orbit gateway run --replace \
  > /tmp/orbit-gateway.log 2>&1 &
