...
description: "Build Bloomreach scenario JSON (clipboard format) for a human to paste and activate."
version: 1.0.0
...
```json
{
  ...
  "sender_email": "orbit-seed-1@email.com",
...
{"scenarioId":"orbit_demo","appVersion":"v1.310.0-0-g8cfbd920","copiedData":{
 "nodes":[
  ...
  {"id":2,"type":"send-email-action","version":1,"consent_category":"other","frequency_policy":"unlimited-policy","transfer_identity":true,"recipient_email":"","recipient_name":"",
   "design":{"type":"raw","data":{"subject":"…","preheader":"…","jinja_html":"<…shell…>","sender_email":"orbit-seed-1@email.com","sender_name":"Pacific Apparel","provider":{"type":"channel_extension","channel_extension_id":"6a1720ff1a26b57a6b5c97f6"},"file_ids":[],"translations":{}}},
   "x":520,"y":300}
 ],
...
Reference: `scenarios/generated/orbit_cartrecovery.json` is a real verified branched export (trigger→email→wait→condition→follow-up email) — copy its connector indices. Flow wiring confirmed: trigger out 0, email out 1, wait out 1, condition out 2 (NO branch) → follow-up email.

---

## A/B split reference
For a verified wiring example (connector indices, variants array, connections block),
see `references/ab-split-wiring.md`.

## Gotchas
- **`templates/winback-shell.html` does NOT exist on disk** (verified 2026-06-01 — file is missing from the repo). When the skill references it, author the email HTML inline from scratch. Use `{{customer.first_name}}`, `{{customer.email_headline}}`, `{{customer.email_body}}`, `{{customer.offer_code}}` as Jinja variables. Don't block on the missing template — just build it.
